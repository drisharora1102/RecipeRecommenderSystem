...
DATA_ZIP = "data/processed.zip"

@st.cache_data(show_spinner=False)
def load_data():
    interactions_csv = f"{DATA_DIR}/interactions_clean.csv"
    recipes_csv = f"{DATA_DIR}/recipes_clean.csv"

    if os.path.exists(interactions_csv) and os.path.exists(recipes_csv):
        ...
        with ZipFile(DATA_ZIP) as zf:
            try:
                interactions = pd.read_csv(zf.open("processed/interactions_clean.csv"))
                recipes = pd.read_csv(zf.open("processed/recipes_clean.csv"))
            except KeyError as exc:
                raise FileNotFoundError(
                    "processed.zip is missing required files: "
                    "processed/interactions_clean.csv and/or processed/recipes_clean.csv"
                ) from exc
    else:
        raise FileNotFoundError(
            "Processed data not found. Expected either extracted CSV files under "
            f"{DATA_DIR}/ or archived files in {DATA_ZIP}."
        )
...
def build_models(interactions):
    ...
    if k >= 1:
        ...
        # Tiny datasets cannot be factorized with svds; use user-mean fallback.
        U = np.zeros((n_users, 0), dtype=np.float64)
        sigma = np.array([], dtype=np.float64)
        Vt = np.zeros((0, n_recipes), dtype=np.float64)
...
def get_top_n(user_id, model_name, models, top_n=20):
    ...
    if model_name == "SVD":
        if models["sigma"].size == 0:
            pred_row = np.full(
                models["n_recipes"], models["user_means"][u_idx], dtype=np.float64
            )
        else:
...
def render_recipe_card(rank, recipe_id, score, n_missing, missing_ings, recipe_info):
    ...
    label = f"#{rank}  Â·  {name}  Â·  ⏱ {hours}  Â·  ⭐ {score:.2f}"
...
def main():
    ...
    # ── Load data ───────────────────────────
    has_extracted_data = (
        os.path.exists(f"{DATA_DIR}/interactions_clean.csv")
        and os.path.exists(f"{DATA_DIR}/recipes_clean.csv")
    )
    has_zipped_data = os.path.exists(DATA_ZIP)
    if not (has_extracted_data or has_zipped_data):
        st.error(
            "Processed data not found. Run notebooks 01-02 first to generate either "
            f"`{DATA_DIR}/interactions_clean.csv` + `{DATA_DIR}/recipes_clean.csv` "
            f"or `{DATA_ZIP}`."
        )
        return
