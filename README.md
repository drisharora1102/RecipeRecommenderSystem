...
```cmd
cd /d <path-to-project>\Recipe-Recommender-main
python -m streamlit run app.py --server.address 127.0.0.1 --server.port 8501
```

If you are not in the project folder, run with an absolute path instead:

```cmd
python -m streamlit run <path-to-project>\Recipe-Recommender-main\app.py --server.address 127.0.0.1 --server.port 8501
```
...
4. Render reads `render.yaml`, builds the Docker image, and deploys.
5. Open the generated `onrender.com` URL.

### Notes

- The app binds to Render's `PORT` automatically.
- Keep `data/processed.zip` (or extracted `data/processed/*.csv`) in the repo.
- First startup may take longer because recommendation models are built on launch.
